    ...
    <div class="container">
                ...
                <ul class="list-unstyled">
                    <li>
                        <div class="d-flex align-items-xl-center" style="padding: 10px;">
                            <h6 class="d-xl-flex align-items-xl-center" style="margin: 10px;">Written by:&nbsp;</h6><a
                                    class="d-xl-flex align-items-xl-center" href="character.php?id=<?= $author['Id'] ?>"
                                    style="color: rgb(220,53,69);"><?= $author['Name'] ?></a></div>
                    </li>
                    <li>
                        <div style="padding: 10px;">
                            <h6 class="d-xl-flex align-items-xl-center" style="padding: 10px;margin: 0px;">Audience and
                                Destination:</h6>
                            <p class="d-xl-flex align-items-xl-center"
                               style="margin-left: 30px;"><?= $book['AudienceDestination'] ?></p>
                        </div>
                    </li>
                    <li>
                        <div style="padding: 10px;">
        ...
        <div class="row">
            <div class="col-md-6" style="padding: 20px;margin-bottom: 10px;">
                <a class="text-uppercase border rounded border-dark" href="books.php"
                   style="width: 109px;border-color: green;background-color: rgba(220,53,69,0.77);color: rgba(255,255,255,0.88);font-size: 13px;font-weight: bold;padding: 16px 32px;margin: 0px 15px;">BACK</a>
                <?php if ($_SESSION['admin']): ?>
                    <a class="text-uppercase border rounded border-dark" href="edit_book.php?id=<?= $book['Id'] ?>"
                       style="width: 109px;border-color: green;padding: 16px 32px;background-color: rgba(220,53,69,0.77);color: rgba(255,255,255,0.88);font-size: 13px;font-weight: bold;margin: 0px 15px;">EDIT</a>
                <?php endif ?>
            </div>
        </div>
        <div class="row">
                        ...
                        <div style="padding: 10px;">
                            ...
                            <?php if ($_SESSION['admin'] || (int)$comment['UserId'] === $auth->getUserId()): ?>
                                <form method="post" style="padding: 10px;"
                                      action="process.php?id=<?= $comment['Id'] ?>">
                                    <input type="hidden" name="operation" value="comment">
                                    <input type="hidden" name="table" value="book">
                                    <button class="btn btn-primary" type="submit" name="command" value="delete"
                                            style="height: 23px;width: 53px;padding: 0px;margin: 0px 10px;">Delete
                                    </button>
                                    <?php if ($_SESSION['admin']): ?>
                                        <button class="btn btn-primary" type="submit" name="command" value="hide"
                                                style="height: 23px;width: 53px;padding: 0px;margin: 0px 10px;">Hide
                                        </button>
                                    <?php endif ?>
                                </form>
                            <?php endif ?>
                ...
                <?php if ($auth->isLoggedIn()): ?>
                    <div style="margin: 20px;">
                        <form method="post" style="padding: 10px;" action="process.php">
                            <textarea class="form-control" style="width: 50%;height: 100px;" name="content" required=""
                                      maxlength="5000" placeholder="Add a comment..."></textarea>
                            <input type="hidden" name="operation" value="comment">
                            <input type="hidden" name="table" value="book">
                            ...
                            <button class="btn btn-secondary" type="submit"
                                    style="background-color: rgba(220,53,69,0.83);width: 90px;margin: 10px;">POST
                            </button>
                        </form>
                    </div>
                <?php endif ?>
